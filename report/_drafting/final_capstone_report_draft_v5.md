...
DAMO 699 Capstone Final Report Draft V5
...
Power BI is the official dashboard deliverable for this capstone. The selected architecture is a direct Power BI connection to Azure Databricks serving-layer tables. The exported Power BI dashboard PDF, Page 1 executive screenshot, and dashboard page inventory are now included as final report evidence. The remaining dashboard documentation items are the `.pbix` submission route if required by the course and final serving-layer manifest row counts if available and verified.
...
The `.pbix` source file is a Power BI source file and is not required as a Git-tracked report artifact. If the course requires `.pbix` submission, it should be submitted separately or maintained locally unless the team confirms another route.
