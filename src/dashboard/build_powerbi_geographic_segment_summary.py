"""Build granular Power BI geographic segment summary data.

This Databricks-compatible job creates a filter-compatible geographic serving
artifact for the P04 Power BI page. Unlike ``powerbi_geographic_summary``, which
is intentionally aggregated at geography level, this table keeps the dashboard
filter dimensions needed for interactive slicing by date, AO3 segment, high-risk
flag, and AO2 expected-profit band.

The job does not retrain AO1 or AO2, redefine AO3 logic, expose target/outcome
fields, or rebuild model predictions. It only aggregates governed upstream AO3
outputs at a Power BI-friendly serving grain.
"""
...
GEOGRAPHIC_SEGMENT_GROUP_COLUMNS = (
    "map_location_label",
...
GEOGRAPHIC_SEGMENT_SUMMARY_COLUMNS = GEOGRAPHIC_SEGMENT_GROUP_COLUMNS + (
    "order_count",
    "order_item_count",
...
class PowerBIGeographicSegmentSummaryConfig:
    """Configuration for the granular Power BI geographic segment summary job."""

    ao3_segment_path: str = DEFAULT_AO3_SEGMENT_PATH
    customer_regional_feature_path: str = DEFAULT_CUSTOMER_REGIONAL_FEATURE_PATH
    geographic_segment_summary_output_path: str = DEFAULT_GEOGRAPHIC_SEGMENT_SUMMARY_OUTPUT_PATH
    metadata_output_path: Path = Path(
        os.getenv(
            "DATACO_POWERBI_GEOGRAPHIC_SEGMENT_SUMMARY_METADATA_PATH",
            str(Path.cwd() / "models/dashboard/powerbi_geographic_segment_summary_metadata.json"),
        )
    )
...
def configure_logging() -> logging.Logger:
    ...
    return logging.getLogger("dataco.powerbi_geographic_segment_summary")
...
def read_delta(spark: SparkSession, path: str, config: PowerBIGeographicSegmentSummaryConfig) -> DataFrame:
...
def profit_band_expression() -> object:
    """Return the AO2 expected-profit band expression used for dashboard slicing."""
    predicted_profit = col("ao2_predicted_order_profit")
    return (
        when(predicted_profit < 0, lit("Negative Profit"))
        .when(predicted_profit < 10, lit("$0 - $10"))
        .when(predicted_profit < 25, lit("$10 - $25"))
        .when(predicted_profit < 50, lit("$25 - $50"))
        .when(predicted_profit < 100, lit("$50 - $100"))
        .otherwise(lit("$100+"))
    )


def build_geographic_segment_summary_dataframe(ao3_df: DataFrame, geographic_df: DataFrame) -> DataFrame:
    """Join AO3 scores to geography and aggregate filter-compatible metrics."""
    ...
    enriched_df = (
        ...
        .withColumn("order_date_DateOrders", order_date)
        .withColumn("order_date_key", date_format(order_date, "yyyyMMdd").cast(IntegerType()))
        .withColumn("order_week_key", date_format(order_date, "yyyy-'W'ww"))
        .withColumn("order_month_key", date_format(order_date, "yyyy-MM"))
        .withColumn("ao1_high_risk_flag", col("ao1_high_risk_flag").cast("boolean"))
        .withColumn("ao2_expected_profit_band", profit_band_expression().cast(StringType()))
    )

    grouped_df = enriched_df.groupBy(*GEOGRAPHIC_SEGMENT_GROUP_COLUMNS).agg(
    ...
    return (
        ...
        .withColumn("powerbi_geographic_segment_summary_timestamp_utc", current_timestamp())
        .select(*GEOGRAPHIC_SEGMENT_SUMMARY_COLUMNS)
    )


def validate_geographic_segment_summary_output(df: DataFrame) -> None:
    """Validate granular geographic segment output before writing."""
    row_count = df.count()
    if row_count == 0:
        raise ValueError("Power BI geographic segment summary contains no rows.")

    assert_required_columns(
        df,
        GEOGRAPHIC_SEGMENT_SUMMARY_COLUMNS,
        "Power BI geographic segment summary",
    )
    assert_no_forbidden_targets(df, "Power BI geographic segment summary")

    missing_filter_dimension_rows = df.filter(
        col("ao3_priority_segment").isNull()
        | col("ao1_high_risk_flag").isNull()
        | col("ao2_expected_profit_band").isNull()
        | col("order_date_key").isNull()
    ).count()
    if missing_filter_dimension_rows:
        raise ValueError(f"Rows with missing filter dimensions: {missing_filter_dimension_rows}")
    ...
    if negative_count_rows:
        raise ValueError(f"Negative geographic segment count rows: {negative_count_rows}")
    ...
    if invalid_rate_rows:
        raise ValueError(f"Invalid geographic segment rate rows: {invalid_rate_rows}")

    invalid_profit_band_rows = df.filter(~col("ao2_expected_profit_band").isin(*PROFIT_BAND_ORDER)).count()
    if invalid_profit_band_rows:
        raise ValueError(f"Invalid AO2 expected-profit band rows: {invalid_profit_band_rows}")


def write_delta(df: DataFrame, output_path: str, config: PowerBIGeographicSegmentSummaryConfig) -> None:
    """Write the geographic segment summary as Delta."""
    (
...
def write_metadata(
    *,
    ...
    metadata = {
        ...
        "output_path": config.geographic_segment_summary_output_path,
        "row_count": int(summary_df.count()),
        "coordinate_available_group_count": coordinate_rows,
        "serving_grain": (
            "country/region/state/coordinates/date keys/AO3 segment/"
            "AO1 high-risk flag/AO2 expected-profit band"
        ),
        "filter_compatibility": [
            "DimDate via order_date_key, order_week_key, or order_month_key",
            "AO3 priority segment",
            "AO1 high-risk flag",
            "AO2 expected-profit band",
        ],
    ...
    logger.info("Wrote Power BI geographic segment summary metadata: %s", config.metadata_output_path)


def run_powerbi_geographic_segment_summary(
    config: PowerBIGeographicSegmentSummaryConfig,
    logger: logging.Logger,
) -> None:
    """Build and write the Power BI geographic segment summary table."""
    config = with_repo_defaults(config)
    spark = get_spark_session()

    logger.info("Starting Power BI geographic segment summary build.")
    ...
    logger.info("Geographic segment summary output path: %s", config.geographic_segment_summary_output_path)
    ...
    summary_df = build_geographic_segment_summary_dataframe(ao3_df, geographic_df)
    validate_geographic_segment_summary_output(summary_df)
    write_delta(summary_df, config.geographic_segment_summary_output_path, config)

    written_df = read_delta(spark, config.geographic_segment_summary_output_path, config)
    validate_geographic_segment_summary_output(written_df)
    write_metadata(config=config, summary_df=written_df, logger=logger)

    logger.info("Power BI geographic segment summary build completed with %d rows.", written_df.count())


def main() -> None:
    """Run the Power BI geographic segment summary job."""
