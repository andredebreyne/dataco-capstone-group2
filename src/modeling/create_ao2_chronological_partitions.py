"""Create deterministic AO2 chronological development/test partitions.

This job reads the leakage-safe AO2 Gold analytical table, applies the frozen
project chronological split policy, and writes a partitioned Delta table for
downstream AO2 regression modeling. It does not train models, fit
preprocessing, tune hyperparameters, or use the final test partition.
"""
...
from src.data_engineering.build_gold_ao2_table import GOLD_AO2_OUTPUT_PATH


VOLUME_ROOT = os.getenv("DATACO_VOLUME_ROOT", "/Volumes/workspace/default/raw_data").rstrip("/")

AO2_PARTITION_OUTPUT_PATH = os.getenv(
    "DATACO_AO2_CHRONOLOGICAL_PARTITIONS_OUTPUT_PATH",
    f"{VOLUME_ROOT}/gold/ao2_profitability_chronological_partitions",
)
...
POLICY_REQUIRED_VALUES = {
    ...
    "development_ratio": "0.80",
    "test_ratio": "0.20",
    "boundary_row_formula": BOUNDARY_FORMULA,
...
SUMMARY_COLUMNS = (
    ...
    "target_min_Order_Profit_Per_Order",
    "target_max_Order_Profit_Per_Order",
    "target_avg_Order_Profit_Per_Order",
...
class AO2ChronologicalPartitionConfig:
    """Configuration for AO2 chronological partition creation."""

    gold_input_path: str = os.getenv("DATACO_GOLD_AO2_OUTPUT_PATH", GOLD_AO2_OUTPUT_PATH)
    partition_output_path: str = AO2_PARTITION_OUTPUT_PATH
...
def configure_logging() -> logging.Logger:
    ...
    return logging.getLogger("dataco.ao2_chronological_partitions")
...
def validate_volume_paths(config: AO2ChronologicalPartitionConfig) -> None:
...
def validate_frozen_policy(policy_csv_path: Path) -> None:
    ...
    if invalid_values:
        raise ValueError(f"Frozen split policy contradicts AO2 requirements: {invalid_values}")
...
def assert_required_columns(df: DataFrame) -> None:
    """Validate the AO2 Gold table has columns required for splitting."""
    missing_columns = sorted(column for column in REQUIRED_COLUMNS if column not in df.columns)
    if missing_columns:
        raise ValueError(f"AO2 Gold table is missing split-required columns: {missing_columns}")
...
def assert_target_contract(df: DataFrame) -> None:
    """Validate the AO2 target is complete before partitioning."""
    missing_count = df.select(
        spark_sum(when(col(TARGET_COLUMN).isNull(), 1).otherwise(0)).alias("missing_count")
    ).collect()[0]["missing_count"]

    if missing_count != 0:
        raise ValueError(f"AO2 target contains {missing_count} missing values.")
...
def build_partitioned_dataframe(df: DataFrame) -> tuple[DataFrame, int, int]:
    ...
    total_rows = df.count()
    if total_rows == 0:
        raise ValueError("AO2 Gold table is empty; cannot create chronological partitions.")
...
def write_delta(df: DataFrame, output_path: str, config: AO2ChronologicalPartitionConfig) -> None:
    """Write the AO2 partitioned DataFrame as Delta."""
    (
...
def validate_written_partitions(
    ...
    config: AO2ChronologicalPartitionConfig,
) -> DataFrame:
    """Validate the written AO2 partition Delta table."""
    partitioned_df = read_delta(spark, config.partition_output_path, config.read_format)
    partitioned_count = partitioned_df.count()
    if partitioned_count != total_rows:
        raise ValueError(
            f"AO2 partition row count must equal AO2 Gold. "
            f"Gold: {total_rows}; partitioned: {partitioned_count}."
        )

    assert_unique_keys(partitioned_df, "AO2 chronological partitions")

    missing_source_keys = source_df.select(*JOIN_KEY_COLUMNS).join(
        partitioned_df.select(*JOIN_KEY_COLUMNS),
        list(JOIN_KEY_COLUMNS),
        "left_anti",
    ).count()
    unexpected_partition_keys = partitioned_df.select(*JOIN_KEY_COLUMNS).join(
        source_df.select(*JOIN_KEY_COLUMNS),
        list(JOIN_KEY_COLUMNS),
        "left_anti",
    ).count()
    if missing_source_keys or unexpected_partition_keys:
        raise ValueError(
            "AO2 partition keys do not exactly match AO2 Gold keys. "
            f"Missing from partitions: {missing_source_keys}; "
            f"unexpected in partitions: {unexpected_partition_keys}."
        )

    partition_counts = {
        row[PARTITION_COLUMN]: row["row_count"]
        for row in partitioned_df.groupBy(PARTITION_COLUMN)
        .agg(spark_count(lit(1)).alias("row_count"))
        .collect()
    }
    expected_counts = {
        DEVELOPMENT_LABEL: development_boundary,
        TEST_LABEL: total_rows - development_boundary,
    }
    if partition_counts != expected_counts:
        raise ValueError(
            f"AO2 partition counts do not match frozen boundary. "
            f"Expected: {expected_counts}; found: {partition_counts}."
        )
...
def collect_summary_stats(
    ...
    aggregations = [
        ...
        spark_min(col(TARGET_COLUMN)).alias("target_min_Order_Profit_Per_Order"),
        spark_max(col(TARGET_COLUMN)).alias("target_max_Order_Profit_Per_Order"),
        avg(col(TARGET_COLUMN)).alias("target_avg_Order_Profit_Per_Order"),
...
def format_summary_value(value: Any) -> str:
    """Convert a summary value to a stable CSV string."""
    if value is None:
        return ""
...
def build_summary_rows(
    ...
    """Build AO2 partition summary rows for the committed reference CSV."""
    execution_timestamp = datetime.now(timezone.utc).isoformat()
    ordering_columns_used = "; ".join(ORDERING_COLUMNS)

    def enrich_summary_row(
        summary_level: str,
        partition_label: str,
        stats: dict[str, Any],
    ) -> dict[str, str]:
        ...
        values = {
            **stats,
            ...
            "source_ao2_gold_path": config.gold_input_path,
            "partition_output_path": config.partition_output_path,
...
def write_summary_csv(summary_rows: list[dict[str, str]], summary_csv_path: Path) -> None:
    """Write the small AO2 partition summary CSV artifact."""
...
def run_ao2_chronological_partitioning(
    config: AO2ChronologicalPartitionConfig,
    logger: logging.Logger,
) -> None:
    """Execute AO2 chronological partition creation."""
    spark = get_spark_session()

    logger.info("Starting AO2 chronological partition creation.")
    logger.info("AO2 Gold input path: %s", config.gold_input_path)
    logger.info("AO2 partition output path: %s", config.partition_output_path)
    logger.info("AO2 partition summary CSV path: %s", config.summary_csv_path)
    ...
    try:
        ...
        logger.info("AO2 Gold input contract validation completed.")

        partitioned_df, total_rows, development_boundary = build_partitioned_dataframe(gold_df)
        logger.info("AO2 Gold row count: %s", total_rows)
        ...
        logger.info("AO2 chronological partition Delta write completed.")

        written_df = validate_written_partitions(
            spark,
            gold_df,
            total_rows,
        ...
        logger.info("AO2 chronological partition post-write validation completed.")
        ...
        logger.info("AO2 partition summary CSV written: %s", config.summary_csv_path)
        logger.info("AO2 chronological partition creation completed successfully.")

    except Exception:
        logger.exception("AO2 chronological partition creation failed.")
        raise


def main() -> None:
    """Run AO2 chronological partition creation with default configuration."""
