...
| `Geographic_Summary` | `powerbi_geographic_summary` | `powerbi_geographic_summary.csv` | Aggregated destination geography table for the global map page. |
...
`Geographic_Summary` is generated by
`src/dashboard/build_powerbi_geographic_summary.py` before the serving-layer
registration or CSV export. It aggregates the governed AO3 scored population to
destination geography groups and includes both text location fields and rounded
coordinates.
...
| Column | Purpose |
| --- | --- |
| `map_location_label` | Text location label for Power BI tooltips or fallback geocoding. |
| `map_location_country` | Destination country token. |
| `map_location_region` | Destination region token. |
| `map_location_state` | Destination state token. |
| `map_latitude` | Rounded latitude for coordinate-backed maps. |
| `map_longitude` | Rounded longitude for coordinate-backed maps. |
| `geo_coordinates_available` | Coordinate QA flag. |
| `order_item_count` | Bubble-size metric for map volume. |
| `high_risk_order_rate` | Color metric for AO1 delivery-risk exposure. |
| `avg_ao1_predicted_late_delivery_probability` | Tooltip risk metric. |
| `avg_ao2_predicted_order_profit` | Tooltip profitability metric. |
| `avg_ao3_predicted_margin` | Tooltip margin metric. |

## Modeling Rules

- Power BI must not recreate AO1/AO2 scores, AO3 margins, thresholds, or segment
  assignments.
- Power BI should use `Geographic_Summary` for the global map instead of
  re-aggregating order-level geography inside the report.
