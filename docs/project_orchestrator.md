...
| Power BI Gold dashboard export | `src/dashboard/export_powerbi_gold_tables.py` | Export governed Gold Delta outputs and small reference CSVs into a local Power BI import folder. | Completed AO1/AO2 score Delta table, completed AO3 segment Delta table, completed Power BI geographic summary table, and reference artifacts. | Gitignored CSV files plus `powerbi_export_manifest.json` under `dashboard/exports/`. | Optional and disabled by default; controlled by `RUN_POWERBI_GOLD_EXPORT`. | Runs in Databricks because it reads Delta paths. It does not recreate model scores, retune thresholds, calculate final-test performance metrics, or use target labels. |
