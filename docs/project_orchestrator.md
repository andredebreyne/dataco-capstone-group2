...
| AO3 K-means extension | `src/modeling/run_ao3_kmeans_extension.py` | Test whether a small K-means clustering view adds interpretive value beyond the AO3 2x2 risk-margin matrix. | Completed AO3 segment Delta table from Issue `#42`. | Cluster profiles CSV, quality metrics CSV, assignment sample CSV, findings markdown, documentation update, and metadata JSON. | Optional and disabled by default; controlled by `RUN_AO3_KMEANS_EXTENSION`. | Uses only AO3 decision-time prediction signals. Does not replace AO3, change H3, use target outcomes, or become a dashboard requirement. |
...
| Power BI geographic summary build | `src/dashboard/build_powerbi_geographic_summary.py` | Aggregate governed AO3 scored orders to destination geography groups for the global map page. | Completed AO3 segment Delta table and customer/regional feature Delta table. | Delta table at `gold/powerbi_geographic_summary` plus metadata JSON. | Optional and disabled by default; controlled by `RUN_POWERBI_GEOGRAPHIC_SUMMARY`. | Runs in Databricks because it reads Delta paths. Provides both text location fields and rounded latitude/longitude. Does not use target or realized-outcome fields. |
| Power BI geographic summary validation | `tests/data_validation/validate_powerbi_geographic_summary.py` | Validate the global-map summary schema, target exclusions, coordinate ranges, rates, counts, metadata, and coordinate coverage. | Completed Power BI geographic summary Delta table and metadata JSON. | Console pass/fail result. | Optional and disabled by default; controlled by `RUN_POWERBI_GEOGRAPHIC_SUMMARY` and `RUN_POWERBI_GEOGRAPHIC_SUMMARY_VALIDATION`. | Runs in Databricks because it reads the generated Delta table. |
| Power BI logistics KPI summary build | `src/dashboard/build_powerbi_logistics_kpi_summary.py` | Aggregate historical logistics KPI context with governed AO1/AO2/AO3 pre-dispatch exposure for the logistics risk exposure page. | Completed Silver Delta table, completed AO3 segment Delta table, shipping/product feature Delta table, and customer/regional feature Delta table. | Delta table at `gold/powerbi_logistics_kpi_summary` plus metadata JSON. | Optional and disabled by default; controlled by `RUN_POWERBI_LOGISTICS_KPI_SUMMARY`. | Runs in Databricks because it reads Delta paths. Historical KPI fields are descriptive; predictive exposure fields reuse governed model outputs and do not claim causal intervention impact. |
| Power BI logistics KPI summary validation | `tests/data_validation/validate_powerbi_logistics_kpi_summary.py` | Validate the logistics KPI summary schema, target exclusions, controlled risk/action labels, rates, non-negative counts, and metadata. | Completed Power BI logistics KPI summary Delta table and metadata JSON. | Console pass/fail result. | Optional and disabled by default; controlled by `RUN_POWERBI_LOGISTICS_KPI_SUMMARY` and `RUN_POWERBI_LOGISTICS_KPI_SUMMARY_VALIDATION`. | Runs in Databricks because it reads the generated Delta table. |
| Power BI logistics order KPI detail build | `src/dashboard/build_powerbi_logistics_order_kpi_detail.py` | Create the order-item-level audit layer that aligns AO1 predictions, actual delivery outcomes, AO2 financial context, and AO3 action queues. | Completed Silver Delta table, completed AO3 segment Delta table, shipping/product feature Delta table, and customer/regional feature Delta table. | Delta table at `gold/powerbi_logistics_order_kpi_detail` plus metadata JSON. | Optional and disabled by default; controlled by `RUN_POWERBI_LOGISTICS_ORDER_KPI_DETAIL`. | Runs in Databricks because it reads Delta paths. Actual outcome fields are exposed for audit/reporting only and must not be used as predictors. |
| Power BI logistics order KPI detail validation | `tests/data_validation/validate_powerbi_logistics_order_kpi_detail.py` | Validate the order-level audit schema, grain uniqueness, governed AO1/AO2/AO3 field completeness, probability ranges, binary flags, outcome classifications, and metadata. | Completed Power BI logistics order KPI detail Delta table and metadata JSON. | Console pass/fail result. | Optional and disabled by default; controlled by `RUN_POWERBI_LOGISTICS_ORDER_KPI_DETAIL` and `RUN_POWERBI_LOGISTICS_ORDER_KPI_DETAIL_VALIDATION`. | Runs in Databricks because it reads the generated Delta table. |
| Power BI logistics order KPI detail CSV export | `src/dashboard/export_powerbi_logistics_order_kpi_detail.py` | Export the order-level logistics KPI audit layer to a gitignored CSV fallback for Power BI import or offline review. | Completed Power BI logistics order KPI detail Delta table. | `dashboard/exports/powerbi_logistics_order_kpi_detail.csv` plus export manifest. | Optional and disabled by default; controlled by `RUN_POWERBI_LOGISTICS_ORDER_KPI_DETAIL_CSV_EXPORT`. | Intended as a fallback/import workflow. The Databricks SQL registration path is preferred for live Power BI connection. |
| Power BI logistics order KPI detail Databricks registration | `src/dashboard/register_powerbi_logistics_order_kpi_detail_table.py` | Register the order-level logistics KPI audit layer as a managed Databricks SQL table for Power BI. | Completed Power BI logistics order KPI detail Delta table. | Managed table `powerbi_logistics_order_kpi_detail` plus manifest table. | Optional and disabled by default; controlled by `RUN_POWERBI_LOGISTICS_ORDER_KPI_DETAIL_DATABRICKS_REGISTRATION`. | Publishes the audit layer only. It does not retrain models, retune thresholds, or claim causal intervention impact. |
| Power BI Gold dashboard export | `src/dashboard/export_powerbi_gold_tables.py` | Export governed Gold Delta outputs and small reference CSVs into a local Power BI import folder. | Completed AO1/AO2 score Delta table, completed AO3 segment Delta table, completed Power BI geographic summary table, completed Power BI logistics KPI summary table, and reference artifacts. | Gitignored CSV files plus `powerbi_export_manifest.json` under `dashboard/exports/`. | Optional and disabled by default; controlled by `RUN_POWERBI_GOLD_EXPORT`. | Runs in Databricks because it reads Delta paths. It does not recreate model scores, retune thresholds, calculate final-test performance metrics, or use target labels. |
...
| Power BI Databricks serving-layer registration | `src/dashboard/register_powerbi_databricks_tables.py` | Register governed dashboard artifacts as managed Databricks SQL `powerbi_*` tables for direct Power BI Desktop connection. | Completed AO1/AO2 score Delta table, completed AO3 segment Delta table, completed Power BI geographic summary table, completed Power BI logistics KPI summary table, and reference/report artifacts. | Managed tables under the configured catalog/schema, plus `powerbi_serving_layer_manifest`. | Optional and disabled by default; controlled by `RUN_POWERBI_DATABRICKS_SERVING_LAYER`. | This is the finalized direct Power BI connection path. The CSV export workflow remains available for offline review and fallback. The registration step publishes dashboard-safe projected schemas and does not recreate scores, retune thresholds, calculate final-test performance metrics, or assign AO3 segments. |
| Power BI Databricks serving-layer static validation | `tests/data_validation/validate_powerbi_databricks_serving_layer.py` | Validate the serving-layer script contract, table names, manifest fields, docs references, safe allowlists, and absence of hard-coded personal paths in active code. | Repository files only. | Console pass/fail result. | Manual static validation step. | Does not require live Databricks or PySpark. It complements manual Databricks SQL verification after registration. |
...
- AO3 risk-margin benchmark metadata JSON.
- AO3 risk-margin benchmark insight CSV.
- AO3 operational recommendation matrix CSV.
- AO3 K-means extension metadata JSON.
- AO3 K-means cluster profile CSV.
- Power BI geographic summary Delta table.
- Power BI geographic summary metadata JSON.
- Power BI logistics KPI summary Delta table.
- Power BI logistics KPI summary metadata JSON.
- Power BI logistics order KPI detail Delta table.
- Power BI logistics order KPI detail metadata JSON.
