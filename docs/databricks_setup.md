...
Community Edition runs on a limited single-node environment. This is expected for the project and should be treated as the official development runtime unless the team formally changes the environment standard.
...
df = (
    ...
    .csv("/FileStore/tables/DataCoSupplyChainDataset.csv")
)
```

The exact DBFS path may differ depending on how the file is uploaded. Always confirm the path in the Databricks UI before running the notebook.
